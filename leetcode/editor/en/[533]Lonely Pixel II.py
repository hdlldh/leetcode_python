...
# , find the number of black pixels located at some specific row R and column C th
# at align with all the following rules: 
# 
#  
        ...
        for i in range(m):
            ...
            for j in range(n):
                if picture[i][j] == 'B':
                    ...
                    colCnt[j] += 1
            if cnt == N:
                count[''.join(picture[i])]+=1

        ans = 0
        for row, cnt in count.items():
            if cnt!=N:continue
            for i, p in enumerate(row):
                if colCnt[i]==N and p=='B': ans += cnt
        return ans
